...
```
Create Jest unit tests for Phase 3B (Securities and Accounts) with MOCKED dependencies.

REQUIREMENTS:
- Use Jest with @testing-library/react for components
- Mock ALL external dependencies (Prisma, Clerk, API calls)
- Test file naming: [feature].test_phase3b.js
- DO NOT modify existing test files

CREATE THESE TEST FILES:

1. __tests__/app/securities/page.test_phase3b.js
   - Mock useAuth() to return different user levels
   - Mock API response for securities list
   - Test grid renders with data
   - Test search filtering works
   - Test add button shows for L4/L5 only
   - Test inline edit triggers API call

2. __tests__/app/api/securities/route.test_phase3b.js
   - Mock Prisma client
   - Test GET returns all securities
   - Test POST validates unique ticker
   - Test POST rejects duplicate ticker
   - Test PUT updates existing security
   - Test DELETE requires admin permission

3. __tests__/app/accounts/page.test_phase3b.js
   - Mock getViewableClients() with different permission levels
   - Test L5 sees all accounts
   - Test L4 sees only org accounts
   - Test L2 sees only own accounts
   - Test filter by account type
   - Test benchmark display

4. __tests__/app/accounts/new/page.test_phase3b.js
   - Mock form submission
   - Test ClientAccount requires valid secdexCode
   - Test MasterAccount creation
   - Test fee schedule selection
   - Test validation errors display
   - Test successful submission redirects

5. __tests__/app/api/accounts/route.test_phase3b.js
   - Mock getCurrentUser() for different roles
   - Mock getViewableClients() 
   - Test GET filters by permissions correctly
   - Test POST validates required fields
   - Test POST links to ClientProfile via secdexCode
   - Test POST rejects invalid secdexCode

Include test utilities:
- mockAuth(level) helper
- mockPrismaClient() helper
- createMockSecurity() factory
- createMockAccount() factory
