...
export const clientProfileSchema = z.object({
  ...
  organizationId: z.string().cuid().or(z.literal("")).nullable().optional().transform(val => val === "" ? null : val),
  parentClientId: z.string().cuid().or(z.literal("")).nullable().optional().transform(val => val === "" ? null : val),
...
// Schema for client invitations (without userId since user doesn't exist yet)
...
export function validateSecurityData(data) {
  return validateSchema(securitySchema, data)
}

export function validateAccountData(data) {
  return validateSchema(masterAccountSchema, data)
}
