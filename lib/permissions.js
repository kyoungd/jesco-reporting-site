...
export async function getViewableClients(user) {
  ...
  try {
    switch (userLevel) {
      ...
      case USER_LEVELS.L4_AGENT: {
        // Agents should see Direct Accounts (L2_CLIENT) and their Sub-Clients (L3_SUBCLIENT)
        // but NOT see themselves or other agents/admins
        if (userOrgId) {
          const orgClients = await db.clientProfile.findMany({
            where: {
              organizationId: userOrgId,
              level: { in: [USER_LEVELS.L2_CLIENT, USER_LEVELS.L3_SUBCLIENT] }
            },
            select: { id: true }
          })
          return orgClients.map(client => client.id)
        } else {
          // If no org, show all Direct Accounts and Sub-Clients
          const directAccountsAndSubs = await db.clientProfile.findMany({
            where: {
              level: { in: [USER_LEVELS.L2_CLIENT, USER_LEVELS.L3_SUBCLIENT] }
            },
            select: { id: true }
          })
          return directAccountsAndSubs.map(client => client.id)
        }
      }
...
// Temporary function to support existing API routes
export function checkPermission(userLevel, action, resource) {
  // For testing purposes, allow all operations for admin/agent levels
  if ([USER_LEVELS.L5_ADMIN, USER_LEVELS.L4_AGENT].includes(userLevel)) {
    return true
  }

  // Allow view operations for all authenticated users
  if (action === 'view') {
    return true
  }

  // Restrict create/update/delete to admin/agent levels
  return false
}